...
//                hidden: true,
                ...
                editable:true,
                edittype:'custom', 
                editoptions:{custom_element: ImgUpload2, custom_value: GetImgValue2}
            },
            {
            	label: '조회수',
            	name: 'hits',
...
// 	        if(id && id!==lastsel){
// 	          jQuery('#jqGrid03').restoreRow(lastsel);
// 	          jQuery('#jqGrid03').editRow(id,true);
// 	          lastsel=id;
// 	        }
// 	    },
    ...
    jQuery("#jqGrid03").setGridParam({datatype:'json', page:1, url:'getGridData.php'}).trigger('reloadGrid');
    //jQuery("#jqGridPager03").setGridParam({page:"page", rows: "rows", sord:"desc" , url:'editData.php'}).trigger('reloadGrid');
    
    $('#jqGrid03').navGrid('#jqGridPager03',
...
// editoptions:{ custom_element: ImgUpload  => 파일 업로드 입력필드를 지정하고 el.onchange = UploadFile;를 통해 아래의 UploadFile()를 호출한다.
function ImgUpload2(value, editOptions) {
    var span = $("<span>");
    var hiddenValue = $("<input>",{type:"hidden", val:value, name:"fileName2", id:"fileName2"});
    var image = $("<img>",{name:"uploadImage2", id:"uploadImage2",value:'',style:"display:none;width:80px;height:80px"});
    var el = document.createElement("input");
    el.type = "file";
    el.id = "imgFile2";
    el.name = "imgFile2";
    el.onchange = UploadFile2;
    span.append(el).append(hiddenValue).append(image);
    return span;
}
...
function UploadFile2() {
    $.ajaxFileUpload({
        ...
        success: function(data,status){
            ...
            $("#fileName2").val(data);
            $("#img_name2").val(data);
            $("#uploadImage2").attr("src","./upload_file/" + data);
            $("#uploadImage2").show();
            $("#imgFile2").hide()
        },
        error: function(data, status, e){
...
// editoptions:{ custom_value: GetImgValue 파일명 리턴
function GetImgValue2(elem, sg, value){
    return $(elem).find("#fileName2").val();
}
